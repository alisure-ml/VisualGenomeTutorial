...
from models import (Image, Object, Attribute, Relationship, Region, Graph, QA, QAObject, Synset)
...
def parse_graph(data, image):
    ...
    for rel in data['relationships']:
        relationships.append(Relationship(rel['id'], object_map[rel['subject']], rel['predicate'],
                                          object_map[rel['object']], parse_synset(rel['relationship_canon'])))
        pass

    # Create the Attributes
    for atr in data['attributes']:
        attributes.append(Attribute(atr['id'], object_map[atr['subject']],
                                    atr['attribute'], parse_synset(atr['attribute_canon'])))
        pass
...
def parse_region_descriptions(data, image):
    ...
    if 'region_id' in data[0]:
        ...
        region_id_key = 'id'
    for info in data:
        regions.append(Region(info[region_id_key], image,
                              info['phrase'], info['x'], info['y'], info['width'], info['height']))
    return regions
